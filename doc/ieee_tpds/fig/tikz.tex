%%% NOTE TO REVIEWERS %%%
% This file generates the images used in the article. It is not necessary to compile this file, as the images are included separately in the archive and will be pulled automatically by 'main.tex'. This file is included in the submission for the purposes of data verifiability.
% If you choose to compile this file, note that the '-shell-escape' parameter must be included in the calls for latexmk or pdflatex. Also note that the output 'tikz.pdf' is incidental and not read by the main tex file ('main.tex').
...
\begin{tikzpicture}%
  ...
  % Mirror line
  \path
    ($(mul0)!0.5!(add0)$) --++ (0:0.6) coordinate (mirror1) 
    (cordic2) --++ (180:4.75/2) coordinate (mirror2)
