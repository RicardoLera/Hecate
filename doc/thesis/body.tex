...
% Context - Convolution
Convolution is a mathematical operation that relates two signals into a third, representing how one is modified by the other. The operation is fundamental in digital signal processing by enabling impulse decomposition and pattern recognition~\cite{smith1997scientist}.

% Context - CNNs
Convolutional Neural Network (CNN) is a computational model based on Artificial Neural Networks and capable of finding patterns in input images via discrete convolution. The convolution kernels, which encode the desired patters, are optimized by a backpropagation algorithm, similar to the training of weights and biases of its predecessors. This technology has revolutionized the field of computer vision due to its high accuracy and ability to train over large sets of data~\cite{alzubaidi2021review}.

% Context - Resource-efficiency
The success of CNNs lead to a substantial increase in image processing demands. Algorithms such as \texttt{im2col}~\cite{im2col} restructure convolution as matricial multiplication, enabling efficient processing on GPUs. Despite this, the training and inference of large CNNs remains costly, leading to the development of accelerator technologies, such as Google's TPU~\cite{jouppi2020domain}.

% Context - FPGA
Concerns over power demands and availability lead to new research focused on resource-efficient accelerators implemented on Field-Programmable Gate Array (FPGA) technology.

% Context 3D CNNs and FCONV
CNNs have been successfully applied to both 2D and 3D image recognition. However, the jump to 3D incurs substantially higher workloads due to the quadratic nature of the standard convolution algorithm. In response, more efficient convolution algorithms have been researched and applied to CNN accelerators, achieving high resource-efficiency and throughput. Fourier convolution (also known as FFT convolution) is one such algorithm. While FFT convolution has been proven inneficient for 2D CNNs~\cite{lavin2016fast}, it retains lower assymptotic bounds compared to any current convolution method, increasing its applicability in 3D CNNs.

% State objective
This project evaluates the applicability of FFT convolution in the context of 3D CNNs. We achieve this by implementing an accelerator in FPGA technology via direct RTL synthesis, measuring its throughput and resource-efficiency, and comparing it to current accelerators.
...
This body of work reveals that CNN acceleration is a multifarious problem; methods are presented as complementary parts of highly complex systems. In light of this, while narrowing our search to FPGA-based accelerators, we will also comment on the tangencial advances concerning this theme. 
...
This section describes the three convolution algorithms relevant to 3D CNNs: matrix-multiplication, Winograd, and FFT convolutions.
...
The lack of transformations makes MMX simple to process on GPUs and other hardware designed for graphical processing~\cite{zhou2021characterizing}. It does \textbf{not}, however, reduce the complexity of the algorithm~\cite{lera2023hardware}.
...
Winograd convolution transforms the input and kernel into a domain (more strictly, a \textit{field}) where pointwise multiplication is equivalent to the convolution of the original signals. This is achieved by sampling the input and kernel at a given set of points using transform matrices. This method becomes inefficient for large kernels where the transform matrices are non-trivial~\cite{alam2022winograd}. This is convenient for modern 2D CNNs, as they tend to have many layers with small (usually 3x3) kernels.
...
In 3D CNNs, the transform matrices required for 3D convolution would not be viable for acceleration. Instead, the blocks are sliced across each dimension such that multiple 1D transforms can be performed and added together~\cite{shen2018towards}. Specifically, convolution is defined as
