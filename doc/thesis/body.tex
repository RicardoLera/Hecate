...
CNNs have been successfully applied to both 2D and 3D image recognition. However, the jump to 3D incurs substantially higher workloads, due to the quadratic nature of the standard convolution algorithm. In response, more efficient convolution algorithms have been researched and applied to CNN accelerators, achieving high resource-efficiency and throughput. Fourier convolution (or FFT convolution) is one such algorithm.

Our review of the state-of-the-art revealed the possibility of applying FFT convolution to 3D CNN acceleration. The method has been proven inneficient for 2D CNNs~\cite{lavin2016fast}, however it retains lower assymptotic bounds compared to any current convolution method, increasing its applicability in 3D CNNs.
...
% Present surveys
Several surveys have been published concerning hardware acceleration of CNNs. Mittal et al. broadly reviewed 2D CNN acceleration in 2020~\cite{mittal2020survey} and 3D CNN acceleration in 2021~\cite{mittal2021survey}. %Advancements in hardware~\cite{feng2019computer} as well as FPGA~\cite{abdelouahab2018accelerating} and ASICs~\cite{moolchandani2021accelerating} have also been surveyed, as well as overall optimizations methods~\cite{zhang2019recent}.
...
Digital images are represented as three-dimensional Euclidean spaces. Within this space, convolution can be defined as the ``sliding product'' of two inputs: The convolution kernel ``slides'' across the input, multiplying each pair of coincident points and adding the products together. Each of these results represents a convolution centered on a different input point, and together form the output of the convolution. This is represented by Equation~\ref{eq:slide}.
...
The lack of transformations makes MMX simple to process on GPUs and other hardware design for graphical processing~\cite{zhou2021characterizing}. It does \textbf{not}, however, reduce the complexity of the algorithm~\cite{lera2023hardware}.
