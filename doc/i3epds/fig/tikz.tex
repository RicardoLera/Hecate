...
\begin{tikzpicture}
  \begin{semilogyaxis}[
    ...
    \addplot[
      ...
      %% 3*Ns*(N/2+1 + sum(N/2^j-2,1<j<log2N))
      4*(
        ((
...
\begin{tikzpicture}
  \begin{semilogyaxis}[
    ...
    \addplot[
      ...
      ceil((2310-x)/2)*ceil((2310-x)/2)*ceil((2310-x)/2)*((2+x-1)^3)
    };
    \addplot[
      samples=9,
      draw opacity=1,
      ...
      %% 3*Ns*(N/2+1 + sum(N/2^j-2,1<j<log2N))
      4*ceil(2310/x)*ceil(2310/x)*ceil(2310/x)*(
        ((
          pow(2,ceil(log2((2*x-1)^3)))
        )/2)+1
        + (pow(2,ceil(log2((2*x-1)^3)))
    ...
    \addplot[
      ...
      %% Ns*(N/2+1 + sum(N/2^j-2,1<j<log2N))
      ceil(2310/x)*ceil(2310/x)*ceil(2310/x)*(
        ((
          pow(2,ceil(log2((2*x-1)^3)))
        )/2)+1
        + (pow(2,ceil(log2((2*x-1)^3)))
...
\begin{tikzpicture}%
  ...
  \node[
    ...
    yshift=-0.2cm]
    at ($(bin)!0.5!(bout0)$) {};
  \node[text=red,above,xshift=2.5cm,yshift=2.3cm] at (bin) {DIT-FFT};
  ...
  % IFFT MUX
  % \path (csplit3) --++ (-90:1) coordinate (ifmux);
  % \draw 
  %   (ifmux) --++
  ...
  % IFFT Butterfly
  \draw
    (csplit3) --++ (0:0.3) coordinate (ifcomp) -- (ifcomp |- bfly0) --++ (0:1) coordinate (bflyi0)
    (ifcomp) -- (ifcomp |- bfly1) node[circle,draw,fill=white]{$\bar{z}$} --++ (0:1) coordinate (bflyi1)
  ...
  \node[
    ...
    yshift=-0.2cm]
    at ($(biin)!0.5!(biout0)$) {};
  \node[text=red,above,xshift=0.2cm,yshift=2.3cm] at (biin) {DIF-FFT};
