...
architecture synth of vivado_tb_3x3x3 is
  ...
  signal hec_reset, hec_start, hec_ready : std_logic;
  signal sxi, syi, szi : natural := 0; 

  file   read_file : text;
  signal state     : t_vivado_tb_state := initial;
  signal res       : t_signed_4d_real_array(0 to kx)(0 to oz-1)(0 to oy-1)(0 to ox-1); -- (66+3−1)×(66+3−1)×(3+3−1)×32×64 = 47349760 b (45 MB)
  ...
  fsm : process (clock) begin
    if rising_edge(clock) then 
      if (reset) then
        ...
        state <=
          -- slice_fft  when (state = initial   ) or  (state = hold)    else
