...
architecture synth of flux_inverter is
  ...
  constant m_0              : std_logic_vector(24 downto 0) := (0 => '1', others => '0');
  signal   m                : std_logic_vector(24 downto 0) := m_0;
  signal   inp_inv, nex_inv : std_logic_vector(23 downto 0);

begin

  b(23) <= m(24);

  gen_b : for i in 22 downto 0 generate
    b(i) <= b(i + 1) or m(i + 1);
  end generate gen_b;

  filtered <= not b and m(23 downto 0) and inp_inv;
  compared <= ((inp_inv xor nex_inv) and (b or m(23 downto 0)));

  gen_inv : for i in 23 downto 0 generate
    inp_inv(i) <= inp(23 - i);
    nex_inv(i) <= nex(23 - i);
  ...
  gen_or : for i in 1 to 23 generate
    filtered_or(i) <= filtered_or(i - 1) or filtered(i);
    compared_or(i) <= compared_or(i - 1) or compared(i);
  end generate gen_or;

  s_error <= compared_or(23);
  erro    <= s_error;

  m_n(0)           <= filtered_or(23);
  m_n(24 downto 1) <= m(23 downto 0);
  s_ready          <= m(24);
  ready            <= s_ready;
