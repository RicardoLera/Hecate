...
architecture synth of flux_multiplier is
  ...
  signal kx_mux_x   : ieee.numeric_std.unsigned((2 * lut_size * size) - 1 downto 0) := (OTHERS => '0');
  signal kx_add_x   : ieee.numeric_std.unsigned((2 * lut_size * size) - 1 downto 0) := (OTHERS => '0');
  signal kx_reg_x   : ieee.numeric_std.unsigned((2 * lut_size * size) - 1 downto 0) := (OTHERS => '0');
  signal kx_shift_x : ieee.numeric_std.unsigned((2 * lut_size * size) - 1 downto 0) := (OTHERS => '0');
  signal kx_mux_y   : ieee.numeric_std.unsigned((2 * lut_size * size) - 1 downto 0) := (OTHERS => '0');
  signal kx_add_y   : ieee.numeric_std.unsigned((2 * lut_size * size) - 1 downto 0) := (OTHERS => '0');
  signal kx_reg_y   : ieee.numeric_std.unsigned((2 * lut_size * size) - 1 downto 0) := (OTHERS => '0');
  signal kx_shift_y : ieee.numeric_std.unsigned((2 * lut_size * size) - 1 downto 0) := (OTHERS => '0');
...
clock,
 ...
 b_error
  ...
  with a_bit select b_sel <=
    ieee.numeric_std.unsigned(b_flux) when '1',
    to_unsigned(0, size + 1) when OTHERS;

  sum <= a_sel + b_sel;

  shift_sum(size + 1 DOWNTO 1)              <= std_logic_vector(sum);
  shift_sum(0)                              <= '0';
  shift_sum((2 * size) - 1 DOWNTO size + 2) <= (OTHERS => '0');
...
p_full_shifted,
 ...
 p_full_n
  ...
  -- Constant multipĺiers (KX)

  kx_x : for idx IN 0 to lut_size - 1 generate
  begin
    ...
    end process kx_proc_x; -- kx_proc_x
  ...
  kx_y : for idx IN 0 to lut_size - 1 generate
  begin
    ...
    end process kx_proc_y; -- kx_proc_y
    ...
    kx_shift_y(2 * (idx + 1) * size - 1 DOWNTO 2 * idx * size) <= kx_reg_y(2 * (idx + 1) * size - 1 downto 2 * idx * size);
