use work.hecate_pkg.all;

library ieee;
  use ieee.std_logic_1164.all;
  use ieee.numeric_std.all;

entity hecate_oa_tb_debug is
end entity hecate_oa_tb_debug;

architecture sim of hecate_oa_tb_debug is

  signal clk, reset, start : std_logic := '0';
  signal o_ready           : std_logic;
  signal res               : b25_3d_real_array(0 to 4)(0 to 4)(0 to 4);

  signal keep_simulating   : std_logic := '0';
  constant clockperiod     : time      := 1 ms;

begin

  clk <= (not clk) and keep_simulating after clockperiod / 2;

  dut : component hecate_oa
    generic map (4, 4, 4, 5, 5, 5)
    port map (
      img     => test_img,
      ker     => test_ker,
      clock   => clk,
      reset   => reset,
      start   => start,
      res     => res,
      o_ready => o_ready
    );

  test : process begin

    keep_simulating <= '1';
    reset           <= '0';
    start           <= '1';

    wait until o_ready for 500 ms;
    wait for 7 * clockperiod;
    report integer'image(to_integer(unsigned(res(3)(3)(3))));
    start <= '0';
    reset <= '1';

    wait for 5 * clockperiod;
    keep_simulating <= '0';
    
    wait;

  end process test;

end architecture sim;
